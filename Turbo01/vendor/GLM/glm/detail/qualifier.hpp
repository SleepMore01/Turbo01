...
#	if GLM_HAS_ALIGNOF
		...
		{
			typedef struct alignas(L * sizeof(T)) m_CarType {
				T data[L];
			} m_CarType;
		};
		...
		{
			typedef struct alignas(4 * sizeof(T)) m_CarType {
				T data[4];
			} m_CarType;
		};
#	endif
...
#	endif
	...
	template<typename genType, genTypeEnum m_CarType>
	struct init_gentype
	{
	};
