...
namespace detail
{
	...
	/*
		...
		T			= m_CarType of scalar values (e.g. float, double)
...
namespace glm
{
	...
	//
	// Swizzles are distinct types from the unswizzled m_CarType.  The below macros will
	// provide template specializations for the swizzle types for the given functions
	// so that the compiler does not have any ambiguity to choosing how to handle
	// the function.
	//
	// The alternative is to use the operator()() when calling the function in order
	// to explicitly convert the swizzled m_CarType to the unswizzled m_CarType.
	//
